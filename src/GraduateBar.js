...
export default{
    ...
    methods:{
    fetchData : function(){
        axios.get('https://data.gov.sg/api/action/datastore_search?resource_id=115bf8a7-46df-466c-b7fc-375ef3c1b425')
        .then(response=>{
        this.results=response.data.result.records;
        for(let key in this.results){
            if (this.results[key].year > 2010 && this.results[key].sex == 'MF'){
